    ...
    <div class="sub-main-w3">
      <form>
        ...
        <div class="form-style-agile">
          ...
          <input placeholder="请输入用户名" v-model="user.name" name="Name" type="text">
        </div>
        <div class="form-style-agile">
          <label>
          ...
          <input placeholder="请输入密码" name="Password" v-model="user.password" type="password">
        </div>
        <!-- checkbox -->
...
export default {
  ...
  methods: {
    ...
    toLogin() {
      ...
      for (let index = 0; index < this.userInfoList.length; index++) {
        if (this.user.name === this.userInfoList[index].username) {
          if (this.user.password === this.userInfoList[index].password) {
            this.$emit("on-login", this.user);
            localStorage.setItem("userInfo", JSON.stringify(this.user));
            this.$Message.success("登录成功");
            return false;
...
.sub-main-w3 form {
  ...
  flex-wrap: wrap;
  justify-content: center;
  border-bottom: 8px solid rgba(15, 166, 255, 0.3);
